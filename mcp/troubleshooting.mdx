...
<AccordionGroup>
  ...
  <Accordion title="A tool is refused because a permission was not granted">
    The approval screen has two optional permissions, **Create and update** and
    **Delete**, and a tool that needs one you left unticked is refused with a
    message naming it. Nothing is broken and nothing half happened.

    To grant it, remove the connector and add it again, ticking the permission
    this time. See [Authentication](/mcp/authentication#what-you-approve). An API
    key connection carries every permission, so this cannot happen on one.
  </Accordion>
  ...
  <Accordion title="Claude shows the connector as unauthorized">
    Adding NeetoRecord from
    [Anthropic's connector directory](https://claude.ai/directory/neetorecord)
    is only half of it. The connector still has to be authorized against your
    workspace before it can be used. Open
    **[Customize > Connectors](https://claude.ai/customize/connectors)**, remove
    NeetoRecord, and add it again if the authorization did not complete. One
    connector list backs claude.ai, the Claude desktop app, Claude mobile, and
    Cowork, so fixing it in one fixes it everywhere.

    On a Team plan the connector may be waiting on an admin instead. A
    **Requested** button means your request has not been acted on yet.
  </Accordion>
</AccordionGroup>
