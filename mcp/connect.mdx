...
<Tabs>
  <Tab title="Claude">
    ...
    **[Settings > Connectors](https://claude.ai/customize/connectors)**, click
    ...
    **[Settings > Connectors](https://claude.ai/customize/connectors)** and
    remove NeetoRecord.
    ...
    <Accordion title="Adding it as a custom connector instead">
      ...
      2. Name it NeetoRecord and paste the server URL:
         ...
         <Frame caption="The Add custom connector dialog, ready to submit.">
           <img src="/assets/mcp/claude-add-custom-connector-dialog.png" alt="Claude's Add custom connector dialog with the name NeetoRecord and the server URL https://connect.neetorecord.com/mcp/messages filled in, and empty optional OAuth client fields under Advanced settings" />
         </Frame>
  ...
  <Tab title="ChatGPT">
    ...
    4. Leave **Authentication** on **OAuth**. ChatGPT reads NeetoRecord's OAuth settings from the URL, so **Advanced OAuth settings** needs nothing from you.
    5. Tick **I understand and want to continue**, then click **Create** and approve the NeetoRecord sign in.
  ...
  <Tab title="VS Code">
    ...
    This requires VS Code 1.99 or later with GitHub Copilot in **Agent** mode. Approve the NeetoRecord sign in when it prompts.
  ...
  <Tab title="Windsurf">
    ...
    Enable the server under **Settings → Cascade → MCP Servers**, then approve the NeetoRecord sign in when it prompts.
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Connect your workspace">
    Enter the subdomain of the workspace you want the assistant to reach. For `acme.neetorecord.com`, enter `acme`. See [Workspace subdomain](/api/workspace-subdomain).
...
Ask the assistant something only the server can answer, for example *"List my five most recent NeetoRecord recordings."* If it answers with real recordings, the connection works. Find more prompts in [Examples](/mcp/examples), and if nothing comes back, see [Troubleshooting](/mcp/troubleshooting).
