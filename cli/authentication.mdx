...
not use the `X-Api-Key` header that the [REST API](/api/authentication)
...
session to `~/.config/neetorecord/auth.json`. If the browser does not open, the
command prints the URL to visit. The sign in must finish within two minutes,
after which `login` gives up and can be run again. Running it again for the
same subdomain refreshes that entry.
